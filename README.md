...
本项目聚焦于多传感器时间同步技术，旨在解决 Livox 激光雷达与海康相机在数据采集时的时间同步问题，以实现多传感器数据在时间轴上的精准对齐，主要涵盖以下关键内容：

**方案一**：参考 fast livo 的工作，通过串口向 Livox Avia LiDAR 发送假的 GPRMC 消息，利用共享内存将 LiDAR 帧头时间戳赋给相机消息。<u>但该方案存在图像时间戳并非真实曝光时间的问题。</u>

**方案二**：在 Livox 驱动中，通过 rostopic 将 image  触发的时间戳发送给相机节点，以此为基础赋值时间戳（其中，“image触发的时间戳“来自stm32的pwm波上升沿触发相机采样的时间戳，所以是真实的图像采集的时间戳），再利用 ros 的 message_filters 功能，按时间最近邻原则对齐点云和 image 帧。具体实施过程中遇到<u>触发信号时序、时间戳回退以及 livox 原始驱动时间戳异常</u>等问题，经调整接线方式、优化 stm32  代码逻辑、设置阈值舍弃异常数据并采用插值算法对剩余数据处理等手段得以解决。

**方案三**：对方案二进行优化，采用共享内存和互斥锁的方法替代 rostopic 通信，<u>补偿相机曝光时间和 $gprmc 信息传输时间，以 gprmc 的 utc 时间为基准计算图像时间戳</u>。经 kalibr 和 open - vins 标定，**该方案能将时间同步误差控制在14ms以内**，为后续数据融合提供可靠基础。

**时间同步误差的测量与标定**：运用 kalibr 标定相机内参，allan_variance_ros 标定IMU噪声，再进行相机-IMU联合标定，获取时间同步误差。实验结果表明，D455 相机与内置 IMU 时间同步误差极小，海康相机与 livox 内置 IMU  在优化前时间同步误差较大，优化后显著降低。

**解决时间戳回退问题**：针对时间戳回退问题，经研究发现是 stm32 定时器与 livox 内部计时器准确率差异所致。通过调整 stm32 代码$gprmc和$cam消息的发送方式及波特率，使 livox 内部计时器计算的 1S 和 stm32 发送 $gprmc 数据的时间间隔均为 1S，消除了时间戳回退现象，确保了时间同步的稳定性。

**解决信号传输问题**：针对 stm32 发送的 pps 信号无法直接触发 livox 的 GPS  同步问题，通过分析发现是信号类型不匹配。将 3.3V LVTTL 电平的 pps 信号经正确方式连接到 Livox 设备后，成功实现了 GPS + PPS 时间同步，使点云时间戳能准确赋值为 $GPRMC 里面的时间戳。

**数据处理优化**：在数据处理过程中，针对 livox 原始驱动时间戳异常问题，通过设置阈值舍弃异常数据，并采用插值算法对剩余数据处理，一定程度上优化了 imu 数据采样的时间间隔，使其更符合理想状态，提高了数据质量。



**实现的时间同步效果**：

![image-20240306215653325](./assets/image-20240306215653325-1744815140686-1.png)
