...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    while (ros::ok()) {
      ...
      //msg->header.stamp = ros::Time::now();// 原来的   用rostime系统时间作为相机的采样时间

      //下面这段我改的。
      ros::Time current_time = ros::Time::now();
      ros::Duration elapsed_time = current_time - start_time;
      msg->header.stamp =ros::Time().fromSec(elapsed_time.toSec()) ;//相对时间

      // 将ros::Time对象转换为秒数
double stamp_sec = msg->header.stamp.toSec();

// 打印时间戳的秒数值
ROS_INFO("Time stamp in seconds: %f", stamp_sec);
