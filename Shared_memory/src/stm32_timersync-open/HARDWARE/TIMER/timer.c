...
char snum[7];
vu16 shorttt=0;
//定时器3的中断时间间隔为10ms，即每10ms执行一次中断函数TIM3_IRQHandler()。
void TIM3_IRQHandler(void)   //TIM3中断
{
		...
		shorttt=0;
		//将global_time转换为一个字符串，格式为"%06d"，表示将global_time转换为6位的十进制整数形式，并在前面补零。转换后的结果被存储在字符数组snum中。
		sprintf(snum, "%06d", global_time); //产生："000011D7"
		snum[6]=0;

		//GPRMC数据格式如下：$GPRMC,014600.00,A,2237.496474,N,11356.089515,E,0.0,225.5,310518,2.3,W,A*23
		//其中，“014600.00“表示UTC时间, 格式hhmmss.ss，代表时分秒.毫秒
		//调试助手看消息
		printf("$GPRMC,");
		printf(snum);
